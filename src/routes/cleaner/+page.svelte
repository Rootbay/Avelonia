<script lang="ts">
  ...
  // Per-tab selection removed; unified selection only
  ...
  // Disk info removed with tabs view
  ...
  const allItems = $derived.by<CleanerItem[]>(() => {
    ...
    function tryPush(it: CleanerItem) {
      ...
      if (seen.has(p)) return; // de-duplicate by path to avoid keyed each duplicates
      seen.add(p);
      items.push(it);
    }
  ...
  // Reset cap when filters/search change significantly
  $effect(() => {
    const _fk = filterKind; const _q = qDeb; const _exc = exclusions.length;
    unifiedCap = 3000;
  });
  ...
  // Temp list scroll tracking removed with tabs view
  ...
  function scheduleLargeFlush() {
    ...
    const run = () => {
      ...
      try {
        ...
        if (scrollingUp) { largeFlushRaf = setTimeout(run, 300) as unknown as number; return; }
      } catch {}
  ...
  function scheduleDupFlush() {
    ...
    const run = () => {
      ...
      try {
        ...
        const userActive = now - _unifiedLastScrollTs < 600;
        const scrollingUp = _unifiedLastDir === 'up' && userActive;
        if (scrollingUp) { dupFlushRaf = setTimeout(run, 300) as unknown as number; return; }
      } catch {}
  ...
  function scheduleEmptyFlush() {
    ...
    const run = () => {
      ...
      try {
        ...
        const userActive = now - _unifiedLastScrollTs < 600;
        const scrollingUp = _unifiedLastDir === 'up' && userActive;
        if (scrollingUp) { emptyFlushRaf = setTimeout(run, 300) as unknown as number; return; }
      } catch {}
  ...
  function scheduleShortcutFlush() {
    ...
    const run = () => {
      ...
      try {
        ...
        const userActive = now - _unifiedLastScrollTs < 600;
        const scrollingUp = _unifiedLastDir === 'up' && userActive;
        if (scrollingUp) { shortcutFlushRaf = setTimeout(run, 300) as unknown as number; return; }
      } catch {}
  ...
  const UNIFIED_PREBUFFER = 3; // render a few rows above the scroll position
  ...
  // Disable virtualization by default to avoid scroll locking in some environments.
  // You can set unifiedAutoFallback = false to re-enable when stable.
  ...
  const unifiedBottomPad = $derived(
    unifiedVirtualize
      ? Math.max(0, (allItems.length * UNIFIED_ROW_PX) - (unifiedTopPad + (unifiedDisplayed.length * UNIFIED_ROW_PX)))
      : 0
  );
  ...
  // Reset position on major context changes (filters, search, exclusions)
  $effect(() => { void filterKind; void qDeb; void exclusions.length; unifiedStart = 0; _unifiedLastScrollTop = 0; });
  ...
  function onUnifiedScroll(_event: Event) {
    ...
    requestAnimationFrame(() => {
      ...
      if (unifiedVirtualize) {
        ...
        const first = approx - UNIFIED_PREBUFFER; // small lookahead, avoid jumpiness
  ...
  // Per-tab scan functions removed
  ...
  <Card>
    <CardHeader>
      ...
      <div class="flex flex-col gap-3">
        ...
        <div class="rounded-md border h-[60vh] overflow-auto" style="overscroll-behavior: contain; overflow-anchor: none;" bind:this={unifiedContainer} on:scroll={onUnifiedScroll}>
