            ...
            <div class="flex items-center gap-2">
              ...
              {#if releaseOptions.length > 1}
                <div onclick={(e) => e.stopPropagation()} role="none" class="shrink-0">
                  ...
                  >
                    <SelectTrigger class="h-6 w-35 overflow-hidden px-2 py-0 text-[10px] uppercase tracking-wider">
          ...
          <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-start overflow-hidden">
            <span class={`text-xs font-semibold uppercase tracking-wide shrink-0 ${statusTone}`}>
              ...
              {#if download.status === 'downloading' && download.speed}
                <span
                  class="ml-1 text-muted-foreground whitespace-nowrap hidden md:inline"
                  >- {download.speed}
                </span>
              {/if}
            </span>
            <div class="flex shrink-0 items-center gap-2">
              ...
              {:else if cancelDownload &&
                ...
                <Button type="button" variant="destructive" size="sm" class="h-7" onclick={cancel}>Cancel</Button>
              {/if}
            </div>
          </div>
        </TableCell>
